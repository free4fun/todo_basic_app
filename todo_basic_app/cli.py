"""This module provides the To-Do Basic App CLI."""
...
from todo_basic_app import (
    ERRORS, __app_name__, __version__, config, database, todo_basic_app
)
...
def get_todoer() -> todo_basic_app.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "todo_basic_app init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return todo_basic_app.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "todo_basic_app init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
...
def add(
    ...
    date: datetime = typer.Option(None,"--date", "-d",help="The limit date and time."),
    priority: int = typer.Option(3, "--priority", "-p", min=1, max=5),

) -> None:
    """Add a new to-do with a DESCRIPTION."""
    todoer = get_todoer()
    todo, error = todoer.add(description, date, priority)
    if error:
        typer.secho(
...
def list_all() -> None:
    ...
    if len(todo_list) == 0:
        typer.secho(
            "There are no tasks in the to-do list yet", fg=typer.colors.YELLOW
        )
        raise typer.Exit()
    ...
    for id, todo in enumerate(todo_list, 1):
        desc, priority, done, created = todo.values()
        typer.secho(
            f"{id}{(len(columns[0]) - len(str(id))) * ' '}"
            f"| ({priority}){(len(columns[1]) - len(str(priority)) - 4) * ' '}"
            f"| {done}{(len(columns[2]) - len(str(done)) - 2) * ' '}"
            f"| {datetime.fromisoformat(created).strftime('%d/%m/%Y')}"
            f"| {desc}",
